...
- Log an error with `errorLogAttributes` from [`lib/logError.ts`](./src/renderer/lib/logError.ts), never by hand. It writes the flat dotted `error.type`, `exception.*` names Core also uses, so one grep finds every stack in a log file. A log record must never carry mutation `variables` (an Entry's is the content the user wrote) or anything unclonable like the QueryClient, which would fail the structured clone and lose the record silently.
